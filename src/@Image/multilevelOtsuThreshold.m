function [regions, thresholds, maxSig] = multilevelOtsuThreshold(obj, nClasses, varargin)
%  Multilevel Thresholding using Otsu Method.
%
%   CLASSES = multilevelOtsuThreshold(IMG, NC)
...
%
%   [CLASSES, THRESHOLDS] = multilevelOtsuThreshold(IMG, NC)
...
%     [classes, threshs] = multilevelOtsuThreshold(img, 3);
...
%
%   See also
%     otsuThreshold, histogram, maxEntropyThreshold
%
...
[h, levels] = histogram(obj, varargin{:});
h = h / sum(h);
...
% compute array of region labels
regData = zeros(size(obj), 'uint8');
for i = 1:nClasses
    inds = obj >= thresholds(i) & obj < thresholds(i+1);
    regData(inds) = i;
end

% create result image
newName = createNewName(obj, '%s-OtsuClasses');
regions = Image(...
    'Data', regData, ...
    'Parent', obj, ...
    'Type', 'Label', ...
