...
function test_createFromArray(testCase) %#ok<*DEFNU>
...
function test_createFromArray3D(testCase)
...
% get a pixel
assertEqual(testCase, uint8(7), img.getPixel(2, 2, 2));

% get a pixel not on diagonal
assertEqual(testCase, uint8(10), img.getPixel(2, 3, 1));


function test_createColor(testCase)

dat = imread('peppers.png');
dim = size(dat);
img = Image.create(dat, 'vector', true);

assertEqual(testCase, 2, ndims(img), 'Wrong dimension for color image');
assertEqual(testCase, dim([2 1]), size(img), 'Wrong size for color image');
